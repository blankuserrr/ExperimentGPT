...
router.post("/sendMessage/:chatId", checkAuth, async (req: RequestWithIo & RequestWithCustomSession, res: Response) => {
...
router.post("/register", async (req: Request, res: Response) => {
	...
	try {
		...
		if (!uid) {
			throw new Error("UID not found after registration");
		}
		console.log("User registered successfully with UID:", uid); // Log the user UID

		const userRef = doc(firestore, `users/${uid}`);
		await setDoc(userRef, { chats: [] }); // This line creates the user document in Firestore
...
router.post("/createChat", checkAuth, async (req: RequestWithCustomSession, res: Response) => {
	try {
		console.log("User ID:", req.uid); // Log the user ID
		const now = new Date();
		...
		await setDoc(chatRef, {
			messages: [],
			name: chatName,
			ownerId: req.uid, // Include the ownerId field here
		});
		...
		const userRef = doc(firestore, "users", req.uid);
		const userDoc = await getDoc(userRef);
		console.log("User document exists:", userDoc.exists()); // Log whether the user document exists
		if (!userDoc.exists()) {
...
router.post("/clearChat/:chatId", checkAuth, async (req: Request, res: Response) => {
	...
	await setDoc(chatRef, { messages: [], name: chatName, ownerId: req.uid });
	res.status(200).send();
});
