...
router.get("/", checkAuth, async (req, res) => {
    const userRef = (0, lite_1.doc)(firebaseConfig_1.firestore, `users/${req.uid}`);
    ...
    const chats = await Promise.all(chatIds.map(async (chatId) => {
        const chatRef = (0, lite_1.doc)(firebaseConfig_1.firestore, "chats", chatId);
...
router.get("/chat/:chatId", checkAuth, async (req, res) => {
    const chatRef = (0, lite_1.doc)(firebaseConfig_1.firestore, 'chats', req.params.chatId);
    const chatDoc = await (0, lite_1.getDoc)(chatRef);
...
router.post("/createChat", checkAuth, async (req, res) => {
    try {
        ...
        const chatRef = (0, lite_1.doc)((0, lite_1.collection)(firebaseConfig_1.firestore, 'chats'));
...
router.post("/deleteChat", checkAuth, async (req, res) => {
    ...
    try {
        await (0, lite_1.deleteDoc)((0, lite_1.doc)(firebaseConfig_1.firestore, `chats/${chatId}`));
        const userRef = (0, lite_1.doc)(firebaseConfig_1.firestore, `users/${req.uid}`);
        const userDoc = await (0, lite_1.getDoc)(userRef);
        if (!userDoc.exists()) {
            throw new error_1.BadRequestError('User does not exist');
